<template>
  <div class="student-advice-sent-content">
...
import { Plus, Picture, Search, Loading } from '@element-plus/icons-vue'
...
const searchUsers = async (query) => {
  ...
  try {
    const response = await request.get(`/api/user/search?name=${encodeURIComponent(query)}`)
  ...
  try {
    ...
    try {
      if (quillEditorRef.value) {
        // 使用 Quill 实例获取 HTML 内容
        const editor = quillEditorRef.value.getEditor()
        if (editor) {
          // 优先使用 root.innerHTML 获取 HTML 内容
          if (editor.root) {
